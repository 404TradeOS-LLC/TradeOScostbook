...
- Configure infrastructure-level network controls (security groups/ALB rules) around platform provisioning per deployment — app-level rate limiting and an optional IP allowlist are already in place; this is the one piece of "production rollout" work that's genuinely deployment-specific and can't be automated from inside this repo.
...
- Once this repo actually has a GitHub remote, configure the `production` GitHub Environment's required reviewers (Settings → Environments) so `.github/workflows/deploy-migrations.yml` enforces human approval before a real rollout, and confirm a real `workflow_dispatch` run end-to-end (only verified locally via `act` so far, never against GitHub's actual infrastructure).
- Consider seeding a few more realistic common assembly templates (a deck package, a bathroom remodel, a kitchen refresh) now that the `isTemplate` mechanism exists — only one ("Residential Driveway Base Package") was added so far, just enough to prove the feature works.
- Consider whether template assemblies need their own admin-shell UI page (browse/manage templates) once a frontend exists — right now they're API-only, consistent with the rest of this backend-only MVP.
