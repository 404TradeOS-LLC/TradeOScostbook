...
## Next Up
- Configure deployment-level network restrictions (security groups/ALB rules) around the platform provisioning route — app-level rate limiting and an optional IP allowlist are now in place; migration/role rollout is now automated and wired into a CI/CD workflow too; this is the one remaining piece that's genuinely deployment-specific.
...
- Once this repo has a real GitHub remote: configure the `production` Environment's required reviewers, and run `.github/workflows/deploy-migrations.yml` for real (only verified locally via `act` so far).
