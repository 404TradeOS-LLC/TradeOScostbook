...
- **Brand:** 404 TradeOS — web design & marketing agency for the trades, and TradeOS — its Construction Intelligence Platform product. Aesthetic = industrial / diagnostic software: forge-black surfaces, a single **copper** accent (#B87333), Space Grotesk headlines, Roboto Mono technical labels. "404" is part of the name — always in the logo lockup.
- **Read `readme.md`'s "Brand Strategy & Identity System" section first** — it indexes the full Brand Bible, Document Design System, Brand Studio spec, and Industrial Design Language grammar, plus the current logo status (original lockups in `assets/logos/` are canonical; the Equipment Nameplate in `assets/logos/v2/` is a pending, not-yet-approved direction).
