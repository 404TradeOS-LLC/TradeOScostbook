...
**None required.** Space Grotesk and Roboto Mono both ship here as real variable TTFs (`assets/fonts/`) with `@font-face` rules in `tokens/fonts.css`. Body is the system sans stack — no file needed.

---

## Brand Strategy &amp; Identity System (read these before designing anything new)

Beyond the visual foundations above, this project also carries a full brand-strategy layer built across several working sessions. Read in this order for context; each builds on the last and none contradict it:

1. **`BRAND-AUDIT.md`** — Phase 1: strengths to preserve, real inconsistencies found, recommendations only (nothing redesigned).
2. **`BRAND-ARCHITECTURE.md`** — Phase 2: the 404 TradeOS (Forge/marketing) vs. TradeOS (Blueprint/product) relationship, 15 permanent design principles, product philosophy, the Contractor Experience, shared-DNA-vs-divergence.
3. **`BRAND-BIBLE.md`** — Phase 3: the full source of truth — mission/voice/naming/logo/color/type/spacing/motion/photography/accessibility rules, plus later-added chapters: **§21** Invisible Intelligence + Industrial Experience, **§22** The Tool Belt Philosophy, **§23** Primary Logo vs. Brand Language (the decision lock — see below), **§24** Material Language (forged steel, stainless, copper, black-oxide hardware, concrete, blueprint paper, glass, dark walnut as *staging materials*, never logo material).
4. **`DOCUMENT-DESIGN-SYSTEM.md`** — the two document systems: **System 1** (404 TradeOS's own corporate documents — full Forge/copper) vs. **System 2** (the white-label engine contractors use for their own customers — Frame/Skin split, contractor's brand is the star).
5. **`BRAND-STUDIO.md`** — the in-product settings module contractors use to configure System 2 branding (IA, UX flow, plain-language copy rules, component list).
6. **`INDUSTRIAL-DESIGN-LANGUAGE.md`** — the brand's visual *grammar*: chamfer/bolt/line-weight/tick-ruler/serial-number standards with locked numeric values — what makes anything recognizable as TradeOS even with the logo removed.
7. **`FINAL-REVIEW.md`** — status snapshot from the most recent production sprint: what's production-ready vs. still open.

### Logo status — read before using any logo asset

**`assets/logos/` holds the original, approved production SVGs** (primary/stacked lockups, wordmark, icon marks, favicon, business-card, email-signature) — these are safe to use today.

**`assets/logos/v2/` holds the Equipment Nameplate exploration** (the industrial-nameplate direction developed in `explorations/industrial-identity-system.html` and refined in `explorations/logo-refinement-pass.html`) — **NOT yet promoted to canonical.** Per explicit direction, do not replace the original lockups with this direction until it's signed off. Reference `explorations/` for the full exploration history (motion system, brand asset library, marketing/product mockups, Brand Studio walkthrough) — these are working artifacts, not yet finalized deliverables.
